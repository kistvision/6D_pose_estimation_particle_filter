...
class ParticleFilter():
    ...
    def start(self, itemid, now, img, depth, label, objects_region, dataset="ycb", posecnn_meta="", rois=[], target_t=None):
        ...
        # else:
        # int_mask = np.zeros((self.info['Height'], self.info['Width']))
        # int_mask[mask_label] = 1
        # print(type(int_mask[0,0]))
        # print(type(label[0,0]))
        # print(type(mask_label[0,0]))
        uint8_mask_label = np.zeros((self.info['Height'], self.info['Width']))
        uint8_mask_label[mask_label] = 1
        uint8_mask_label = uint8_mask_label.astype(np.uint8)
        ret, masks, stats, centroid = cv2.connectedComponentsWithStats(uint8_mask_label)
