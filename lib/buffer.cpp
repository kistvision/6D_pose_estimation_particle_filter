...
void VertexBuffer::SetPositions(const std::vector<glm::vec3>& positions) {
	...
	// glm::vec3* h_data = (glm::vec3 *)malloc(sizeof(glm::vec3) * positions.size());
	// cudaMemcpy(h_data, d_positions, sizeof(glm::vec3) * positions.size(), cudaMemcpyDeviceToHost);
	// std::cout << glm::vec3(h_data[0]) << std::endl;
	// std::cout << glm::vec3(h_data[1]) << std::endl;
	// std::cout << glm::vec3(h_data[2]) << std::endl;
}
...
void FrameBuffer::ClearBuffer() {
	...
	// cudaMemset(p_rotation, 0, sizeof(glm::mat3) * num_particles);
	// cudaMemset(p_translation, 0, sizeof(glm::vec3) * num_particles);
